...
public class TestScenarios
{
    ...
    public TestScenarios(AppLauncher launcher, UIAutomationHelper uiHelper, ImageComparer comparer, string outputDir)
    {
        ...
        _dpiScale = uiHelper.DpiScale > 0 ? uiHelper.DpiScale : 1.0;
    ...
    public async Task<List<TestResult>> RunAllTestsAsync(Action<string>? log = null)
    {
        ...
        // Test 6: Fullscreen comparison
        Results.Add(await TestFullscreenAsync(log));

        // Test 7: Button states comparison
        Results.Add(await TestButtonStatesAsync(log));

        // Test 8: Per-region analysis
        Results.Add(await TestRibbonRegionsAsync(log));
    ...
    public async Task<TestResult> TestInitialRibbonStateAsync(Action<string>? log = null)
    {
        ...
        try
        {
            ...
            // Capture ribbon areas (auto-DPI height)
            var installedRibbon = ScreenCapture.CaptureProcessRibbon(_launcher.InstalledProcess);
            var devRibbon = ScreenCapture.CaptureProcessRibbon(_launcher.DevProcess);
    ...
    public async Task<TestResult> TestWindowResizeAsync(Action<string>? log = null)
    {
        ...
        try
        {
            // Test at different sizes (including edge cases for collapsed groups and extra-wide galleries)
            int[] widths = { 600, 800, 1000, 1200, 1400, 1600 };
    ...
    /// <summary>
    /// Test 6: Compare ribbon at fullscreen (maximized) window size
    /// </summary>
    public async Task<TestResult> TestFullscreenAsync(Action<string>? log = null)
    {
        var result = new TestResult { TestName = "Fullscreen" };
        log?.Invoke($"\nRunning: {result.TestName}");
        ...
        try
        {
            ...
            {
                result.Passed = false;
                result.Message = "Applications not running";
                return result;
            }

            // Refresh handles
            _launcher.InstalledProcess.Refresh();
            _launcher.DevProcess.Refresh();
            var installedHandle = _launcher.InstalledProcess.MainWindowHandle;
            var devHandle = _launcher.DevProcess.MainWindowHandle;

            // Maximize both windows
            ScreenCapture.BringToForeground(_launcher.InstalledProcess);
            _uiHelper.MaximizeWindow(installedHandle);
            await Task.Delay(500);

            ScreenCapture.BringToForeground(_launcher.DevProcess);
            _uiHelper.MaximizeWindow(devHandle);
            await Task.Delay(500);

            var installedSize = _uiHelper.GetWindowSize(installedHandle);
            var devSize = _uiHelper.GetWindowSize(devHandle);
            log?.Invoke($"  After maximize: installed={installedSize.Width}x{installedSize.Height}, dev={devSize.Width}x{devSize.Height}");
            ...
            {
                var comparison = _comparer.Compare(installedRibbon, devRibbon);
                _comparer.SaveComparisonReport(_outputDir, "06_Fullscreen", installedRibbon, devRibbon, comparison);

                result.Similarity = comparison.SimilarityPercentage;
                result.Passed = result.Similarity >= 90;
                result.Message = $"Fullscreen similarity: {result.Similarity:F1}%";

                var capturedSize = _uiHelper.GetWindowSize(installedHandle);
                log?.Invoke($"  Window size: {capturedSize.Width}x{capturedSize.Height}");

                if (result.Similarity < 90)
                {
                    result.Differences.Add($"Fullscreen: {result.Similarity:F1}% similar");
                }
            ...
            // Restore windows back to normal for subsequent tests
            _uiHelper.RestoreWindow(installedHandle);
            _uiHelper.RestoreWindow(devHandle);
            await Task.Delay(500);
    ...
    /// <summary>
    /// Test 7: Compare button states and layout
    /// </summary>
    ...
    /// <summary>
    /// Test 7: Per-region ribbon analysis (TabStrip, ContentArea, GroupLabels)
    /// </summary>
    public async Task<TestResult> TestRibbonRegionsAsync(Action<string>? log = null)
    {
        var result = new TestResult { TestName = "Ribbon Regions" };
        log?.Invoke($"\nRunning: {result.TestName}");
        ...
        try
        {
            ...
            // Define standard ribbon regions (in physical pixels of the captured image)
            // These are relative to the window top-left captured by CaptureProcessRibbon.
            // At 96 DPI: titlebar(31) + tab(24) = 55, content(92), grouplabel(16) = 163 total.
            // Scale all boundaries by DPI to handle 125%, 150%, etc.
            int capturedWidth = Math.Min(installedRibbon.Width, devRibbon.Width);
            int tabStripEnd = (int)(55 * _dpiScale);      // titlebar + tab header
            int contentEnd = (int)(147 * _dpiScale);       // tabStripEnd + content(92)
            int groupLabelEnd = (int)(163 * _dpiScale);    // contentEnd + grouplabel(16)
            var regions = new Dictionary<string, Rectangle>
            {
                // Tab strip: title bar + tab headers
                { "TabStrip", new Rectangle(0, 0, capturedWidth, tabStripEnd) },
                // Content area: below tabs, above group labels
                { "ContentArea", new Rectangle(0, tabStripEnd, capturedWidth, contentEnd - tabStripEnd) },
                // Group labels: bottom portion
                { "GroupLabels", new Rectangle(0, contentEnd, capturedWidth, groupLabelEnd - contentEnd) },
            };

            var comparison = _comparer.CompareRegions(installedRibbon, devRibbon, regions);
            _comparer.SaveComparisonReport(_outputDir, "07_RibbonRegions", installedRibbon, devRibbon, comparison);

            result.Similarity = comparison.SimilarityPercentage;
            result.Passed = true;

            // Log per-region results
            if (comparison.RegionResults != null)
            {
                foreach (var (regionName, regionResult) in comparison.RegionResults)
                {
                    var regionStatus = regionResult.SimilarityPercentage >= 90 ? "OK" : "LOW";
                    result.Differences.Add($"{regionName}: {regionResult.SimilarityPercentage:F1}% [{regionStatus}]");
                    log?.Invoke($"  Region '{regionName}': {regionResult.SimilarityPercentage:F1}% similar (MaxDelta={regionResult.MaxChannelDelta}, AvgDelta={regionResult.AvgDifferentPixelDelta:F1})");

                    if (regionResult.SimilarityPercentage < 90)
                    {
                        result.Passed = false;
                    }
                }
            ...
            result.Message = $"Overall: {comparison.SimilarityPercentage:F1}%, {comparison.RegionResults?.Count ?? 0} regions analyzed";
            result.ScreenshotPath = Path.Combine(_outputDir, "07_RibbonRegions_comparison.png");
