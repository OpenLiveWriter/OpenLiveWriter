...
public class UIAutomationHelper
{
    ...
    public void AttachToInstalled(Process? process)
    {
        ...
        // Detect DPI scaling for the installed window
        _installedDpiScale = GetDpiScaleForWindow(process.MainWindowHandle);
        Console.WriteLine($"  Installed DPI scale: {_installedDpiScale:F2} ({_installedDpiScale * 100:F0}%)");
    }
    ...
    public void AttachToDev(Process? process)
    {
        ...
        Console.WriteLine($"  Dev DPI scale: {_dpiScale:F2} ({_dpiScale * 100:F0}%)");

        if (_installedDpiScale != _dpiScale)
        {
            Console.WriteLine($"  WARNING: DPI scale mismatch! Installed={_installedDpiScale:F2}, Dev={_dpiScale:F2}. Windows may be on different monitors.");
        }
    }
    ...
    /// Uses a retry loop to verify both windows end up at the exact same size.
    /// </summary>
    public void ResizeWindowsToMatch(IntPtr installed, IntPtr dev, int width = 1200, int height = 800)
    {
        const int maxAttempts = 3;
        int devX = 50 + width + 20;

        // Position installed on the left
        MoveWindow(installed, 50, 50, width, height, true);
        Thread.Sleep(100);

        // Read the installed window's ACTUAL resulting size (may differ from requested due to DPI, min size, etc.)
        var installedSize = GetWindowSize(installed);
        if (installedSize.IsEmpty)
        {
            Console.WriteLine("  WARNING: Could not read installed window size after resize");
            installedSize = new Size(width, height);
        }

        // Get installed window's actual position for logging
        GetWindowRect(installed, out RECT installedRect);
        int actualInstalledWidth = installedRect.Right - installedRect.Left;
        int actualInstalledHeight = installedRect.Bottom - installedRect.Top;

        // Force dev window to the installed window's actual size with retry
        for (int attempt = 1; attempt <= maxAttempts; attempt++)
        {
            MoveWindow(dev, devX, 50, actualInstalledWidth, actualInstalledHeight, true);
            Thread.Sleep(100);

            var devSize = GetWindowSize(dev);
            if (devSize.Width == installedSize.Width && devSize.Height == installedSize.Height)
            {
                if (attempt > 1)
                    Console.WriteLine($"  Window sizes matched on attempt {attempt}");
                return;
            }
            ...
            Console.WriteLine($"  Resize attempt {attempt}: installed={installedSize.Width}x{installedSize.Height}, dev={devSize.Width}x{devSize.Height}, diff=({widthDiff},{heightDiff})");

            // Adjust the requested size to compensate
            actualInstalledWidth += widthDiff;
            actualInstalledHeight += heightDiff;
            Thread.Sleep(50);
        }

        // Final verification
        var finalInstalled = GetWindowSize(installed);
        var finalDev = GetWindowSize(dev);
        if (finalInstalled != finalDev)
        {
            Console.WriteLine($"  WARNING: Windows still differ after {maxAttempts} attempts: installed={finalInstalled.Width}x{finalInstalled.Height}, dev={finalDev.Width}x{finalDev.Height}");
        }
    }
    ...
    /// <summary>
    /// Restores a window from maximized/minimized state.
    /// </summary>
