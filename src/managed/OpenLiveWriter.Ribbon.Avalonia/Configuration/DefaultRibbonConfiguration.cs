...
namespace OpenLiveWriter.Ribbon.Managed.Configuration
{
    ...
    public static class DefaultRibbonConfiguration
    {
        ...
        private static TabConfig CreateHomeTab()
        {
            ...
            // Paste (large dropdown) + Cut/Copy (small stacked). Group label "Paste" at bottom.
            ...
            fontGroup.Controls.Add(new ColorPickerConfig { CommandId = CommandId.FontColorPicker, ColorTemplate = RibbonColorTemplate.StandardColors });
            ...
            { 
                ...
                MinColumnsLarge = 2  // Show 2 columns at Large to match native ribbon at wider widths
            });
            ...
            // Editing Group - 4 stacked medium buttons matching native TextEditingGroup
            // Native: SizeDefinition="FourButtons" with CheckSpelling, WordCount, FindButton, SelectAll
            ...
            // Scaling policy - matches native ribbon's ScalingPolicy XML exactly.
            // Groups scale down in this precise order as window width decreases.
            tab.ScalingPolicy = new ScalingPolicy();
            // Ideal sizes
            tab.ScalingPolicy.IdealSizes[CommandId.ClipboardGroup] = RibbonGroupSize.Large;
            tab.ScalingPolicy.IdealSizes[CommandId.PublishGroup] = RibbonGroupSize.Large;
            tab.ScalingPolicy.IdealSizes[CommandId.FontGroup] = RibbonGroupSize.Large;
            tab.ScalingPolicy.IdealSizes[CommandId.InsertGroup] = RibbonGroupSize.Large;
            tab.ScalingPolicy.IdealSizes[CommandId.TextEditingGroup] = RibbonGroupSize.Medium;
            tab.ScalingPolicy.IdealSizes[CommandId.ParagraphGroup] = RibbonGroupSize.Medium;
            tab.ScalingPolicy.IdealSizes[CommandId.SemanticHtmlGroup] = RibbonGroupSize.Large;
            // Scale steps (applied in order as width decreases)
            tab.ScalingPolicy.ScaleSteps.Add(new ScaleStep { GroupId = CommandId.ClipboardGroup, Size = RibbonGroupSize.Medium });
            tab.ScalingPolicy.ScaleSteps.Add(new ScaleStep { GroupId = CommandId.SemanticHtmlGroup, Size = RibbonGroupSize.Medium });
            tab.ScalingPolicy.ScaleSteps.Add(new ScaleStep { GroupId = CommandId.TextEditingGroup, Size = RibbonGroupSize.Small });
            tab.ScalingPolicy.ScaleSteps.Add(new ScaleStep { GroupId = CommandId.TextEditingGroup, Size = RibbonGroupSize.Popup });
            tab.ScalingPolicy.ScaleSteps.Add(new ScaleStep { GroupId = CommandId.ClipboardGroup, Size = RibbonGroupSize.Popup });
            tab.ScalingPolicy.ScaleSteps.Add(new ScaleStep { GroupId = CommandId.ParagraphGroup, Size = RibbonGroupSize.Popup });
            tab.ScalingPolicy.ScaleSteps.Add(new ScaleStep { GroupId = CommandId.InsertGroup, Size = RibbonGroupSize.Medium });
            tab.ScalingPolicy.ScaleSteps.Add(new ScaleStep { GroupId = CommandId.SemanticHtmlGroup, Size = RibbonGroupSize.Popup });
            tab.ScalingPolicy.ScaleSteps.Add(new ScaleStep { GroupId = CommandId.FontGroup, Size = RibbonGroupSize.Popup });
