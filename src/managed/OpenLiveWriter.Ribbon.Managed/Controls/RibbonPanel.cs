...
namespace OpenLiveWriter.Ribbon.Managed.Controls
{
    ...
    public class RibbonPanel : UserControl
    {
        ...
        private bool _qatInTitleBar = true; // QAT in title bar to match native ribbon layout
        ...
        private void InitializeComponents()
        {
            ...
            _quickAccessToolbar = new QuickAccessToolbar
            {
                ...
                BackColor = Color.Transparent,
                Visible = !_qatInTitleBar // Hidden when QAT is in title bar (native ribbon style)
            };
        ...
        private RibbonTab CreateTabFromConfig(TabConfig config)
        {
            ...
            // Transfer the scaling policy from configuration
            if (config.ScalingPolicy != null && config.ScalingPolicy.ScaleSteps.Count > 0)
            {
                tab.ScalingPolicy = config.ScalingPolicy;
            }
        ...
        private RibbonControlBase CreateControlFromConfig(ControlConfig config)
        {
            ...
            switch (config)
            {
                ...
                case ComboBoxConfig comboConfig:
                    ...
                    {
                        comboBox.Width = DisplayHelper.ScaleXCeil(comboConfig.PreferredWidth);
                    }
                    control = comboBox;
                    break;
        ...
        {
            // Draw bottom border - native ribbon has a subtle gray line at the bottom
            // separating the ribbon from the document content area
            var g = e.Graphics;
            var borderY = _contentPanel.Height - 1;
            using (var pen = new Pen(RibbonColors.Current.TabBorder))
            {
                g.DrawLine(pen, 0, borderY, _contentPanel.Width, borderY);
            }
        }
        ...
        private void ShowApplicationMenu()
        {
            ...
            if (_applicationMenu != null && _applicationMenu.Visible)
            {
                _applicationMenu.Hide();
                return;
            }
