    ...
    public class RibbonRenderer
    {
            ...
            RibbonContextualTabGroup contextualGroup = RibbonContextualTabGroup.None)
        {
            ...
            if (isSelected)
            {
                ...
                {
                    // Native ribbon uses very subtle 1px corner radius
                    var cornerRadius = DisplayHelper.ScaleYCeil(1);
                    // Left border
                    ...
                    // Small corner connectors (1px diagonal)
        ...
        public void DrawGroup(Graphics g, Rectangle bounds, string label)
        {
            ...
            // Group background - flat color matching ribbon background (no gradient)
            // Native Windows Ribbon uses a uniform background with no visible
            // gradient, so the label area seamlessly blends with the content area.
            var groupBgColor = Colors.GroupBackground.A == 255
                ? Colors.GroupBackground
                : RibbonColors.DefaultOpaqueGroupBackground;
            using (var brush = new SolidBrush(groupBgColor))
            {
                g.FillRectangle(brush, bounds);
            }
            ...
            // Draw vertical separator on right side - single subtle line (matches native ribbon)
            // Native ribbon separators have more generous top/bottom margins - DPI-scaled
            var separatorX = bounds.Right - 1;
            var separatorTopMargin = DisplayHelper.ScaleYCeil(3);
            var separatorBottomMargin = DisplayHelper.ScaleYCeil(4);
            var separatorTop = bounds.Top + separatorTopMargin;
            var separatorBottom = bounds.Bottom - separatorBottomMargin;
            ...
            // Draw label background area if configured (native ribbon uses transparent)
            if (Colors.GroupLabelBackground.A > 0)
            {
            ...
            // Draw top border of label area if configured (native ribbon uses no visible border)
            if (Colors.GroupLabelBorder.A > 0)
            {
            ...
            // Group label at bottom - centered text (native ribbon uses small, subdued text)
            if (!string.IsNullOrEmpty(label))
            {
                ...
                var leftPadding = DisplayHelper.ScaleXCeil(3);
                var rightPadding = DisplayHelper.ScaleXCeil(3); // Space before separator
                var topPadding = DisplayHelper.ScaleYCeil(1);
                var bottomPadding = DisplayHelper.ScaleYCeil(1);
                ...
                // Use 7pt to match native ribbon's compact group labels
                // Font constructor handles DPI scaling automatically via point size
                var fontSize = 7f;
                using (var font = new Font(SystemFonts.MenuFont.FontFamily, fontSize))
                {
            ...
            RibbonButtonType buttonType, RibbonGroupSize size)
        {
            ...
            // Use rounded rectangles for hover/pressed/checked states (native ribbon style)
            var cornerRadius = 2;
            if (borderColor != Color.Transparent && cornerRadius > 0)
            {
                // Draw rounded fill + border for active states
                using (var path = CreateRoundedRectPath(bounds.X, bounds.Y, bounds.Width - 1, bounds.Height - 1, cornerRadius))
                {
            ...
            {
                // Normal state - flat fill, no border
                using (var brush = new SolidBrush(fillColor))
                {
                    g.FillRectangle(brush, bounds);
                }
        ...
        private void DrawLargeButtonContent(Graphics g, Rectangle bounds, string text, Image image, bool isEnabled)
        {
            ...
            if (!string.IsNullOrEmpty(text))
            {
                ...
                // Use the base font size in points - WinForms/GDI+ automatically
                // scales for DPI when rendering. Do NOT manually scale by DpiY.
                var fontSize = 8f;
                using (var font = new Font(SystemFonts.MenuFont.FontFamily, fontSize))
                {
        ...
        private void DrawMediumButtonContent(Graphics g, Rectangle bounds, string text, Image image, bool isEnabled)
        {
            ...
            // Use compact padding to match native ribbon's tight medium button layout
        ...
        /// </summary>
        private static System.Drawing.Drawing2D.GraphicsPath CreateRoundedRectPath(int x, int y, int width, int height, int radius)
        {
        ...
        public void DrawDisabledImage(Graphics g, Image image, Rectangle bounds)
        {
            ...
            bool isSelected, bool isHovered)
        {
            ...
            if (!string.IsNullOrEmpty(text))
            {
                ...
                // Use the base font size in points - WinForms/GDI+ automatically scales for DPI
                var fontSize = 7.5f;
                using (var font = new Font(SystemFonts.MenuFont.FontFamily, fontSize))
                {
        ...
        public void DrawAppMenuButton(Graphics g, Rectangle bounds, bool isHovered, bool isPressed)
        {
            ...
            // "File" text with high-quality rendering - DPI auto-scales
            var fontSize = 9.5f;
            using (var font = new Font(SystemFonts.MenuFont.FontFamily, fontSize, FontStyle.Bold))
            {
            ...
            bool isHovered, bool isSeparatorBefore)
        {
            ...
            // Text with high-quality rendering - DPI auto-scales
        ...
        public void DrawSeparator(Graphics g, Rectangle bounds, bool isVertical)
        {
            using (var pen = new Pen(Colors.Separator))
            {
                if (isVertical)
                {
                    // Native ribbon uses generous top/bottom padding for vertical separators
                    var vPadding = DisplayHelper.ScaleYCeil(3);
                    var x = bounds.X + bounds.Width / 2;
                    g.DrawLine(pen, x, bounds.Y + vPadding, x, bounds.Bottom - vPadding);
                }
                else
                {
                    var hPadding = DisplayHelper.ScaleXCeil(4);
                    var y = bounds.Y + bounds.Height / 2;
                    g.DrawLine(pen, bounds.X + hPadding, y, bounds.Right - hPadding, y);
                }
            }
        }
