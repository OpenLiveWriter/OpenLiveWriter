...
namespace OpenLiveWriter.PostEditor
{
    ...
    public class BlogPostEditingManager : IBlogPostEditingContext, IBlogPostPublishingContext, IDisposable, IBlogPostEditingManager
    {
        ...
        {
            Debug.WriteLine("[OLW-DEBUG] Did not dispose BlogPostEditingManager!");
        }
        ...
        BlogPost IBlogPostPublishingContext.GetBlogPostForPublishing()
        {
            ...
            if (contents != String.Empty)
            {
                ...
                {
                    Debug.WriteLine("[OLW-DEBUG] Exception while trimming whitespace from post: " + e.ToString());
                }
            }
        ...
        void INewCategoryContext.NewCategoryAdded(BlogPostCategory category)
        {
            ...
            {
                Debug.WriteLine("[OLW-DEBUG] Unexpected exception during call to NewCategoryAdded: " + ex.ToString());
            }
        }
        ...
        public void NewPage()
        {
            ...
            if (Blog.ClientOptions.SupportsPages)
                ...
                Debug.WriteLine("[OLW-DEBUG] Attempted to create a Page for a Weblog that does not support pages!");
        ...
        public void DeleteCurrentDraft()
        {
            ...
            if (!PostIsDraft)
            {
                Debug.WriteLine("[OLW-DEBUG] Attempted to delete a post that is not a draft!");
                return;
            }
        ...
        public void EditPostOnline(bool clearPostWindow)
        {
            ...
            {
                Debug.WriteLine("[OLW-DEBUG] Invalid URL provided for online post editing: " + postEditingUrl);
                return;
            }
        ...
        private void AutoSave()
        {
            using (new QuickTimer("AutoSave"))
            {
                ...
                {
                    Debug.WriteLine("[OLW-DEBUG] AutoSave failed! " + ex.Message);
        ...
        private bool PostToWeblog(bool publish)
        {
            try
            {
                ...
                if (UpdateWeblog(publish))
                {
                    ...
                    if (_lastPublishingResult.PostResult.PostId == String.Empty)
                        Debug.WriteLine("[OLW-DEBUG] Warning: PostId is empty after publish");
                    foreach (IBlogPostEditor postEditor in _postEditors)
        ...
        private void BeforePublish(object sender, UpdateWeblogProgressForm.PublishingEventArgs args)
        {
            ...
            {
                if (noPluginsEnabled)
                    Debug.WriteLine("[OLW-DEBUG] No plugins enabled yet header/footer prerequisites met!?");
            ...
            foreach (ContentSourceInfo csi in ContentSourceManager.GetActiveHeaderFooterPlugins(form, Blog.Id))
            {
                ...
                {
                    Debug.WriteLine("[OLW-DEBUG] Unbalanced divs detected in HTML generated by " + csi.Name + ": " + html);
                ...
                if (position == HeaderFooterSource.Position.Header)
                    ...
                    Debug.WriteLine("[OLW-DEBUG] Unknown HeaderFooter position: " + position);
            }
        ...
        {
            if (_mainFrameWindow.InvokeRequired)
                Debug.WriteLine("[OLW-DEBUG] PrePublishHooks invoked on non-UI thread");
        ...
        {
            Debug.WriteLine("[OLW-DEBUG] Plugin exception: " + e.ToString());
        ...
        {
            if (_mainFrameWindow.InvokeRequired)
                Debug.WriteLine("[OLW-DEBUG] PostPublishHooks invoked on non-UI thread");
                    ...
                    new UpdateWeblogProgressForm(_mainFrameWindow, this, BlogPost.IsPage, BlogName, publish))
                {
                    ...
                    else
                    {
                        if (updateWeblogProgressForm.Exception != null)
                            Debug.WriteLine("[OLW-DEBUG] UpdateWeblog failed: " + updateWeblogProgressForm.Exception.ToString());
        ...
        private void SaveToRecentPosts()
        {
            ...
            {
                Debug.WriteLine("[OLW-DEBUG] Unexpected exception saving recent post: " + ex.ToString());
            }
        }
