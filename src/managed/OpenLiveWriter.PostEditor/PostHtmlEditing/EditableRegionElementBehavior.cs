    ...
    internal class EditableRegionElementBehavior : HtmlEditorElementBehavior
    {
        ...
        public virtual string GetEditedHtml(bool useXhtml, bool doCleanup)
        {
            ...
            try
            {
                if (doCleanup || useXhtml)
                {
                    ...
                    if (useXhtml)
                    {
                        ...
                        result = doCleanup ? CleanupHtml(xhtml, true) : xhtml;
                    }
                }
            }
            catch (OutOfMemoryException)
            {
                // For very large documents, CleanupHtml can cause OutOfMemoryException
                // due to repeated StringBuilder allocations. Return the raw innerHTML
                // rather than losing the post entirely (unclean HTML is better than no HTML).
                Trace.WriteLine("OutOfMemoryException during HTML cleanup; returning raw HTML to avoid data loss.");
                return HtmlUtils.FixNestedListHtml(HTMLElement.innerHTML ?? String.Empty);
            }
            catch (Exception e)
            {
            ...
            if (result == null)
            {
                ...
                try
                {
                    result = doCleanup ? CleanupHtml(html, false) : html;
                }
                catch (OutOfMemoryException)
                {
                    Trace.WriteLine("OutOfMemoryException during HTML cleanup; returning raw HTML to avoid data loss.");
            ...
            // Fix invalid nested list HTML generated by MSHTML where <ul>/<ol> elements
            // appear as siblings of <li> instead of being nested inside the preceding <li>.
            result = HtmlUtils.FixNestedListHtml(result);

            return result;
        }

        /// <summary>
        /// Maximum number of cleanup iterations to prevent runaway loops on pathological HTML.
        /// Each iteration rebuilds the entire HTML string with StringBuilder, so unbounded
        /// iterations can cause OutOfMemoryException on large documents.
        /// </summary>
        internal const int MaxCleanupIterations = 30;
        ...
        private string CleanupHtml(string html, bool xml)
        {
            ...
            do
            {
                ...
                iterations++;
            } while (needsCleanup && iterations < MaxCleanupIterations);

            if (iterations >= MaxCleanupIterations)
            {
                Trace.WriteLine("CleanupHtml reached maximum iteration limit (" + MaxCleanupIterations + "); returning partially cleaned HTML.");
            }
