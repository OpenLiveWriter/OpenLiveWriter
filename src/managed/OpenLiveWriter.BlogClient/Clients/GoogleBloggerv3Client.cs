...
namespace OpenLiveWriter.BlogClient.Clients
{
    ...
    public class GoogleBloggerv3Client : BlogClientBase, IBlogClient
    {
        ...
        private string PostNewImage(string albumName, string filename)
        {
            ...
            // Create the MediaItem on GooglePhotos
            var batchCreateResponse = library.MediaItems.BatchCreate(new BatchCreateMediaItemsRequest()
            {
                AlbumId = albumId,
                NewMediaItems = new NewMediaItem[] {
                    new NewMediaItem()
                    {
